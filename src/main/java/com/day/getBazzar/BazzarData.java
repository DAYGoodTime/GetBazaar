...
public class BazzarData {
    ...
    public static void KeepUpdateBazzarData_quick(JSONObject SB_BAZZAR_JSON_FULL) {
        ...
        try {
            ...
            if (id_row_nm == statistics_day + 1) {
                ...
                id_row_nm = 0;
                if (id_row_day % statistics_week == 0) {
                    KeepUpdateBzData_day(SB_BAZZAR_JSON_FULL, TB_WK);
                } else if (id_row_day % statistics_month == 0) {
    ...
    public static List<Object> statisticsData_day(String product_name, String TB_NAME) {
        ...
        try {
            ...
            String sqlMin = "SELECT Min(LowestBuyOderPriceAvg) FROM " + TB_DAY + " where products_name = ?";
            result.add((Db.use(bazzar_ds).query(sqlMin, product_name)).get(0).get("Min(LowestBuyOderPriceAvg)"));
            String sqlMax = "SELECT MAX(HighestSellOderPriceAvg) FROM " + TB_DAY + " where products_name = ?";
            result.add((Db.use(bazzar_ds).query(sqlMax, product_name)).get(0).get("MAX(HighestSellOderPriceAvg)"));
            return result;
        } catch (SQLException e) {
            log.error("日统计数据错误");
            e.printStackTrace();
